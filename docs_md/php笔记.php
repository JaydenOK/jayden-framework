...
//////////////////////////////////////////////////////////////////
/// system(), exec(), 执行linux命令，返回结果。

//popen(): 上面的方法只能简单地执行命令，却不能与命令交互。但有些时候必须向命令输入一些东西，如在增加Linux的系统用户时，要调用su来把当前用户换到root才行，而su命令必须要在命令行上输入root的密码。这种情况下，用上面提到的方法显然是不行的。
...
//及当前用户jay想用root用户增加普通用户。切换root需要输入root密码，使用su切换后的的句柄写入。