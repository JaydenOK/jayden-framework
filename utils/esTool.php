...
class esTool
{
    ...
    public function search()
    {
        ...
        unset($inputArr['node']);
        $client = Elasticsearch\ClientBuilder::create()->setHosts($node)->build();
        $response = $client->search($inputArr);
...
//            file_put_contents(ROOT_DIR . '/data/t.txt', $queryResult['cursor'] . PHP_EOL, FILE_APPEND);
        }
        return $result;
    }
     ...
     * 作者: Jayden
     */
    public static function esInsertBySql($sql, $nodes = 'esList', $size = 10000)
    {
        //$clientListing = \Elasticsearch\ClientBuilder::create()->setHosts($nodes)->build();
        $result = [];
        $i = 0;
        $columns = [];
        $params = [];
        //$queryResult = $clientListing->sql()->translate($params);
        return $result;
    }
    ...
    public static function esSearch($node, $index, $queryBody, $sourceFields = true, $pageSize = 10, $page = 1, $sortArr = [])
    {
        ...
        if (!empty($sortArr)) {
            ...
            foreach ($sortArr as $field => $sort) {
                ...
                $body['sort'][$field] = ['order' => ($sort == 'desc') ? 'desc' : 'asc'];
            }
        }
        if (!empty($queryBody)) {
            //match 会进行分词，将分词后的field去倒排索引寻找文档；
            //term 不会进行分词，将原始的field去倒排索引中寻找文档；
            $body['query'] = $queryBody;
        }
        $params = [
            'index' => $index,
...
//        exit;
     ...
     * @return array
     */
    public static function esSearchNew($node, $index, $queryBody, $sourceFields = true, $pageSize = 10, $page = 1, $sortArr = [])
    {
        $client = \Elasticsearch\ClientBuilder::create()->setHosts($node)->build();
        $page = intval($page);
        $pageSize = intval($pageSize);
...
//            ],
//            'data' => [
//                'like' => 'MMM0181891001@#IMS#@IMS@@65681B65F2087347',
//            ],
//            'accountName' => [
//                'like' => '10tobebet',
//            ],
...
//            ],
    ...
    //search after翻页搜索模式（能实时查询、效率高，但操作麻烦，必须有排序字段，每次要传最后结果的排序id值）
    public static function esSearchAfter($node, $index, $queryBody, $sourceFields = true, $pageSize = 10, $page = 1, $sortArr = [])
    {
        ...
        //todo 这里仅供服务端查询，客户端需传最后的id及pit值
    ...
    //scroll翻页搜索模式（适用于客户端滚动查询，执行简单，无法反映翻页期间数据的变动）
    public static function scrollSearch($node, $index, $queryBody, $sourceFields = true, $pageSize = 10, $page = 1, $sortArr = [], $scrollId = null)
    {
        $client = \Elasticsearch\ClientBuilder::create()->setHosts($node)->build();
        $page = intval($page);
        $pageSize = intval($pageSize);
