...
轻量级消息队列系统，支持 **MySQL** 和 **Redis** 双存储引擎，支持多虚拟主机、多队列、多消费者的分布式消息处理。

### 功能特性

- **双存储引擎**：支持 MySQL 和 Redis 存储，按虚拟主机配置自动切换
- 多虚拟主机：不同虚拟主机连接不同数据库/Redis，实现业务隔离
...
### 效果图
  ![图1](/system/bin/p1.png "图1") 
  ![图2](/system/bin/p2.png "图2") 
...
**说明**：系统会根据 `adapter` 字段自动选择存储引擎，`mysql` 使用数据库存储，`redis` 使用 Redis 存储。
...
当需要确保消息唯一性或覆盖已有消息时，可以指定 `msgId`：

```php
// 使用数组格式：[$mqName, $msgId]
// 如果 msgId 已存在，则覆盖原消息
MqManager::set(['queueName', 'order_123'], $data);

// msgId 可以是任意唯一字符串
MqManager::set(['syncQueue', 'user_456_sync'], $userData);
```

**使用场景**：
- 订单状态同步：使用订单ID作为 msgId，状态变更时覆盖之前的消息
- 数据同步：使用业务主键作为 msgId，避免重复同步
- 去重处理：相同 msgId 只保留最新的消息

### 延迟消费

发送消息时指定延迟秒数，消息在指定时间后才能被消费者获取：
...
// 延迟 1 小时后消费
MqManager::set(['reminderQueue', null, 3600], $reminderData);
```

**使用场景**：
- 订单超时取消：下单后30分钟未支付自动取消
- 延迟通知：用户操作后延迟发送提醒
- 定时任务：指定时间后执行某些操作
...
MqManager::set(
    ['orderTimeoutQueue', "timeout_{$orderId}", 1800],  // 30分钟 = 1800秒
    ['orderId' => $orderId, 'action' => 'check_payment']
);

// 如果用户在30分钟内支付，覆盖为立即执行的确认消息
MqManager::set(
    ['orderTimeoutQueue', "timeout_{$orderId}"],  // 不指定延迟，立即可消费
...
system/
├── config/
│   ├── db.php           # 数据库/Redis配置（生产环境）
│   ├── db-dev.php       # 数据库/Redis配置（开发环境）
│   └── mq.php           # 队列配置
├── data/
│   └── mq/
│       ├── lock/        # 进程锁文件
│       └── log/         # 消费者日志
├── DbManager.php        # MySQL数据库连接管理器
├── RedisManager.php     # Redis连接管理器
├── MqController.php     # 控制器（Web界面、CLI命令）
├── MqManager.php        # 队列管理器（核心类，支持MySQL/Redis双引擎）
├── bin/
...
### Q: MySQL 和 Redis 存储有什么区别？

| 特性 | MySQL | Redis |
|-----|-------|-------|
| 持久化 | 天然持久化 | 依赖 Redis 持久化配置 |
| 性能 | 中等 | 高 |
| 适用场景 | 消息量中等，需要强持久化 | 高并发，对性能要求高 |
| 查询能力 | 支持复杂查询 | 有限 |

系统会根据虚拟主机的 `adapter` 配置自动选择存储引擎。

### Q: 延迟消息的精度如何？

延迟消息的精度取决于消费者的轮询间隔，通常在秒级。消费者会定期检查延迟队列，将到期的消息移入待处理队列。
